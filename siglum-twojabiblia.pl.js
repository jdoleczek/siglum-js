...
Siglum.prepare = function(book) {
    var str = [];
    book = book.toLowerCase();

    for (var i = 0; i < book.length; i++) {
        if (book.charCodeAt(i) < 65) {
            str.push(book.charAt(i));
        } else {
            str.push(book.charAt(i).toUpperCase());
            str.push(book.slice(i + 1));
            break;
        }
    }
    ...
    if (!node.tagName) {
        ...
        if (!!match.length) {
            ...
            for (var i = 0; i < parts.length; i++) {
                ...
                addr = siglum.substr(book.length).replace(/[ \t]/ig, '').split(',');
                aSiglum = document.createElement('a');
                aSiglum.title = Siglum.booksLowerCase[book] + '\nrozdział: ' + addr[0] + (addr[1] ? '\nwers: ' + addr[1] : '');
                aSiglum.href = 'http://twojabiblia.pl/czytaj/' + Siglum.prepare(book.replace(/ /ig, '')) + '/' + addr[0];
